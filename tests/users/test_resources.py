...
from apps.messages import MSG_NO_DATA, MSG_INVALID_DATA, MSG_PASSWORD_WRONG
...
class TestSignUp:
    ...
    def test_message_when_password_is_not_valid(self, client):
        ...
        assert resp.json.get('errors').get('confirm_password') == MSG_PASSWORD_WRONG
    ...
    def test_responses_ok(self, client, mongo):
        resp = client.post(
            self.ENDPOINT,
            data=dumps(dict(full_name='teste', email='teste1@teste.com', password='123456', confirm_password='123456')),
            content_type='application/json'
        )
        print(resp.json)
